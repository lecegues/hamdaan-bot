...
class YoutubeCog(commands.Cog):
    ...
    @discord.slash_command(name="play", description="Play the selected song from YouTube")
    async def play(self, ctx, query: str): 
        ...
        if voice_channel is None: 
            ...
            if song == False: # Check if the search failed
                await ctx.respond("Could not download the song. Incorrect format, try a different keyword") 
            else: 
    ...
    @discord.slash_command(name="leave", description="Disconnect the bot from the voice channel")
